...
from browser_utils import cleanup_profile_templates
from models import (
...
async def lifespan(app: FastAPI):
    ...
    # 在清理实例之前，先清理未使用的模板目录
    try:
        # 假定你的模板目录都在 'profiles' 文件夹下
        base_profile_path = Path.cwd() / "profiles"
        all_sites_config = get_config().llm_sites
        cleanup_profile_templates(base_profile_path, all_sites_config)
    except Exception as e:
        logger.error(f"在关闭时清理 Profile 模板目录失败: {e}")
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    """
    捕获 Pydantic 模型验证失败的异常，记录详细的请求体，然后返回标准的422响应。
    """
    ...
    try:
        ...
        log_message = (
            f"请求验证失败 (422): {request.method} {request.url.path}\n"
            f"客户端: {request.client.host}\n"
        )

        try:
            body_json = json.loads(body_str)
            log_message += f"请求体 (JSON):\n{json.dumps(body_json, indent=2, ensure_ascii=False)}"
        except json.JSONDecodeError:
            log_message += f"请求体 (Raw):\n{body_str}"
