...
from browser_utils import robust_click
...
class BrowserInstance:
    ...
    def __init__(self, browser_context: Optional[BrowserContext], page: Optional[Page], browser_pid: Optional[int] = None, is_mock: bool = False):
        ...
        self.temp_profile_dir_path: Optional[Path] = None
    ...
    async def cleanup(self):
        ...
        try:
            ...
            if self.temp_profile_dir_path and self.temp_profile_dir_path.exists():
                logger.info(f"Waiting 3 seconds before attempting to remove temporary profile directory: {self.temp_profile_dir_path}")
                await asyncio.sleep(3) # 按要求等待3秒
                
                for attempt in range(3): # 尝试3次
                    try:
                        shutil.rmtree(self.temp_profile_dir_path, ignore_errors=False)
                        logger.info(f"Successfully removed temporary profile directory on attempt {attempt + 1}.")
                        break # 成功，退出循环
                    except Exception as e_rm:
                        if attempt < 2: # 如果不是最后一次尝试
                            logger.warning(f"Attempt {attempt + 1} to remove directory failed: {e_rm}. Retrying in 1 second...")
                            await asyncio.sleep(1) # 等待1秒后重试
                        else:
                            logger.error(f"Failed to remove temporary profile directory {self.temp_profile_dir_path} after multiple attempts: {e_rm}")
...
class LLMWebsiteAutomator:
    ...
    async def _create_single_browser_instance(self) -> BrowserInstance:
        ...
        unique_temp_dir_path: Optional[Path] = None
        ...
        try:
            ...
            base_profile_path_str = self.config.firefox_profile_dir
            base_profile_path = Path(base_profile_path_str)
            if not base_profile_path.is_absolute():
                base_profile_path = Path.cwd() / base_profile_path_str
            
            if not base_profile_path.exists():
                logger.warning(f"[{self.config.id}] Base profile template not found at {base_profile_path}. Creating it.")
                base_profile_path.mkdir(parents=True, exist_ok=True)
            else:
                logger.info(f"[{self.config.id}] Using base profile template from: {base_profile_path}")

            unique_temp_dir_path = Path(tempfile.mkdtemp(prefix=f"{self.config.id}_instance_"))
            logger.info(f"[{self.config.id}] Created unique temporary profile directory: {unique_temp_dir_path}")

            shutil.copytree(base_profile_path, unique_temp_dir_path, dirs_exist_ok=True)
            
            user_data_dir = str(unique_temp_dir_path)
            ...
            bi.temp_profile_dir_path = unique_temp_dir_path
            return bi
        except Exception as e:
            ...
            if playwright_obj and self._playwright_instance:
                 ...
                 self._playwright_instance = None
            if unique_temp_dir_path and unique_temp_dir_path.exists():
                shutil.rmtree(unique_temp_dir_path, ignore_errors=True)
            raise
